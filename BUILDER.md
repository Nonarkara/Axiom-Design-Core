...
Axiom Design Core is the law half. This file is the other half, and the two meet
at three points:

- **Register before prototype.** §XV picks the register — Console, Index, Civic,
  Editorial, Institutional — and it is a decision about who is reading and in
  what condition. That is §2's "imagine the human" applied to a surface. Make it
  before the first rough build, not after.
- **The thing you tear apart is a composition, not a token set.** Tokens are
  settled. What is genuinely uncertain in a new surface is the structure — which
  skeleton, what dominates, where the one bold move lands. Prototype that.
- **`npx axiom-audit .` is the machine half of §4.** It fails the build on a
  banned font, a second accent, a stray radius, a leaked key. It cannot tell you
  whether the thing is any good. Walk it with an agent behaving like a person for
  that part.
